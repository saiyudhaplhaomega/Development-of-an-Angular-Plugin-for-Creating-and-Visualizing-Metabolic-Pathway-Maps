...
@Component({
  ...
  template: `
    ...
    <ng-template #matIcon>
      <span *ngIf="props.info" [ngStyle]="{ cursor: 'pointer' }">
        <mat-icon>info</mat-icon>
        <span class="tooltip-text">{{ props.info }}</span>
      </span>
    </ng-template>
  `,
})
export class FormlyWrapperAddons extends FieldWrapper implements AfterViewInit {
  ...
  addonInfoClick($event: any) {
    if (this.props.info.onClick) {
      this.props.info.onClick(this.to, this, $event);
    }
  }
}
