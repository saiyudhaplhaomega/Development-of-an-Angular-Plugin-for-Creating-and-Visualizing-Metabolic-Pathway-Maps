...
export class MainComponent implements OnInit {
  ...
  doubleClicked(event: MouseEvent) {
    ...
    if (clickedNode) {
        ...
        const nextLabelIndex = this.hierarchy.findIndex((h)=> h==clickedNode.level);
        const nextLabel = this.hierarchy[nextLabelIndex+1];
  ...
  tick(ctx: CanvasRenderingContext2D, nodes: any[]) {
    ...
    if (this.brushEnabled) {
      ...
      ctx.rect(this.brushX0 * this.zoomScale, this.brushY0 * this.zoomScale, (this.brushX1 - this.brushX0) * this.zoomScale, (this.brushY1 - this.brushY0) * this.zoomScale);
  ...
  private brushended(selectedNodes: any) {
    let s = this.isNodeInBrush(this.nodesData);
    ...
    console.log('selectedNodes', scaledBrushX0, scaledBrushX1, scaledBrushY0, scaledBrushY1);
  ...
  dragged(event: any, d: any) {
    this.dragNode = true;
    if (this.brushEnabled) {
        this.brushX1 = event.x / this.zoomScale;
        this.brushY1 = event.y / this.zoomScale;
        console.log('event.x / this.zoomScale', event.x, this.zoomScale, this.brushX1);
