...
export class SpectrumViewerComponent implements OnInit {
  ...
  handleSpectrumServiceData(dataObject: SpectrumDataObject) {
    ...
    this.scatterChartData.datasets[1].data = this.prepareData(
      dataObject.ySingle
    );
    this.scatterChartData.datasets[2].data = this.prepareData(
      dataObject.yDouble
    );
    this.scatterChartData.datasets[3].data = this.prepareData(
      dataObject.bSingle
    );
    this.scatterChartData.datasets[4].data = this.prepareData(
      dataObject.bDouble
    );
    this.chart?.update();
  }
  ...
  prepareData(localRawData: DataPoint[]) {
    // creates 2 dataPoints for every dataPoint in the rawData -> 1 in front, 1 behind on the x-Axis
  ...
  toggleY1() {
    // sets border and backgroundColor so that when datapoint is hovered over the correct color is displayed
    this.showY1 = !this.showY1;
    this.scatterChartData.datasets[1].borderColor =
      this.showY1 === false ? this.defaultColor : this.y1Color;
  ...
  toggleY2() {
    ...
    this.scatterChartData.datasets[2].borderColor =
      this.showY2 === false ? this.defaultColor : this.y2Color;
  ...
  toggleB1() {
    ...
    this.scatterChartData.datasets[3].borderColor =
      this.showB1 === false ? this.defaultColor : this.b1Color;
  ...
  toggleB2() {
    ...
    this.scatterChartData.datasets[4].borderColor =
      this.showB2 === false ? this.defaultColor : this.b2Color;
